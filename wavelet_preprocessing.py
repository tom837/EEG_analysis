...
def wavelet_denoise(signal, wavelet='db4', level=1, threshold_method='soft'):
...
def wavelet_denoise_rt(signal, wavelet='db4', level=1, threshold_method='soft'):
...
def wavelet_rt(eeg_data):
    ...
    for trial in eeg_data:
        ...
        # Convert cleaned trial back to 2D array and store it in the cleaned data dictionary
        cleaned_trial = np.array(cleaned_trial).T  # Transpose to match original structure
        eeg_cleaned_data.append(cleaned_trial)
    return np.array(eeg_cleaned_data)
